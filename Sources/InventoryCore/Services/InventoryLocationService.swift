...
public protocol InventoryLocationService: Sendable {
    /// Returns all items directly in a given space (Room/Building/Volume).
    func getItems(in space: any InventorySpace) async throws -> [any InventoryItem]
    
    /// Returns all containers in a given space.
    func getContainers(in space: any InventorySpace) async throws -> [any ItemContainer]
    ...
    func resolveGeoLocation(for space: any InventorySpace) async -> InventoryGeoLocation?
