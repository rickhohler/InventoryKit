...
public struct SchemaVersion: Codable, Equatable, Comparable, CustomStringConvertible, Sendable {
    ...
    public init(_ version: String) throws {
        ...
        guard let regex = try? NSRegularExpression(pattern: pattern, options: []) else {
            throw SchemaVersionError.internalError("Failed to compile regex.")
        }
        ...
        guard let match = regex.firstMatch(in: version, options: [], range: range) else {
            throw SchemaVersionError.invalidFormat(version)
        }
    ...
    public func isCompatible(with version: SchemaVersion) -> Bool {
        major == version.major
    }
...
public enum SchemaVersionError: Error, Equatable {
