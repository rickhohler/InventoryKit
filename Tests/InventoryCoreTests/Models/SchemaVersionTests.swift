...
final class SchemaVersionTests: XCTestCase {
    func testSchemaComparison() throws {
        ...
        let v1_patch = SchemaVersion(major: 1, minor: 0, patch: 1)
        ...
        let alpha = SchemaVersion(major: 1, minor: 0, patch: 0, prerelease: "alpha")
        let beta = SchemaVersion(major: 1, minor: 0, patch: 0, prerelease: "beta")
    ...
    func testStringInit() throws {
        // Valid
        let v1 = try SchemaVersion("1.0.0")
        XCTAssertEqual(v1.major, 1)
        XCTAssertEqual(v1.minor, 0)
        XCTAssertEqual(v1.patch, 0)
        
        let vComplex = try SchemaVersion("2.3.4-alpha+build.123")
        ...
        // Invalid
        XCTAssertThrowsError(try SchemaVersion("invalid")) { error in
             XCTAssertEqual(error as? SchemaVersionError, .invalidFormat("invalid"))
        }
        XCTAssertThrowsError(try SchemaVersion("1.2")) // Missing patch
    }
    ...
    func testCodable() throws {
        let v = SchemaVersion(major: 2, minor: 0, patch: 5)
        let data = try JSONEncoder().encode(v)
        let decoded = try JSONDecoder().decode(SchemaVersion.self, from: data)
