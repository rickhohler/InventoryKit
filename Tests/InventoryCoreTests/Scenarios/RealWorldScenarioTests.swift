...
final class InventoryRealWorldScenarioTests: XCTestCase {
    ...
    func testUltimaIVScenario() {
        ...
        struct MockItem: InventoryItem {
            ...
            // Custom init to match the test usage roughly, or just rely on memberwise
            init(name: String, type: String, typeClassifier: ItemClassifierType) {
                self.name = name
                self.typeIdentifier = type
                self.typeClassifier = typeClassifier
            }
        }
        ...
        let digitalItems = mutableCopy.children.filter { $0.typeClassifier.category == ItemClassifierType.InventoryCategory.digital }
        XCTAssertEqual(digitalItems.count, 3) 
        
        let physicalItems = mutableCopy.children.filter { $0.typeClassifier.category == ItemClassifierType.InventoryCategory.physical }
        ...
        XCTAssertEqual(mutableCopy.children.last?.typeClassifier.category, ItemClassifierType.InventoryCategory.physical)
    }
}
